import { useGetMe, useGetMyStats, useGetMyStreak } from "@workspace/api-client-react";
import { Loader2, Flame, Star, CheckCircle, BookOpen, Edit2, Zap } from "lucide-react";
    ...
    <div className="max-w-3xl mx-auto space-y-8 pb-12">
      ...
      <div className="p-6 rounded-3xl border-2 border-b-4 border-amber-300 bg-gradient-to-br from-amber-50 to-yellow-100 dark:from-amber-950/30 dark:to-yellow-900/20 dark:border-amber-800">
        <div className="flex items-end justify-between mb-3">
          <div>
            ...
            <div className="text-3xl font-black text-foreground mt-1">
              {xpInLevel} <span className="text-amber-600 dark:text-amber-400">/ {XP_PER_LEVEL}</span>
              <span className="ml-2 text-base font-bold text-muted-foreground uppercase">XP</span>
            </div>
          </div>
          <div className="text-right">
            <div className="text-xs font-bold text-muted-foreground uppercase">Дараагийн түвшин</div>
            <div className="text-lg font-black text-amber-600 dark:text-amber-400">{xpToNext} XP үлдсэн</div>
          </div>
        </div>
        ...
        <p className="mt-3 text-sm font-medium text-amber-800/80 dark:text-amber-300/80">
          Цуваагаа үргэлжлүүлэхэд илүү XP олно — өдөр бүр сурснаар бонус нэмэгдэнэ.
        </p>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard icon={Flame} color="orange" value={stats.longestStreakDays} label="Хамгийн урт цуваа" />
          <StatCard icon={Star} color="yellow" value={stats.totalXp} label="Нийт XP" />
          <StatCard icon={CheckCircle} color="green" value={stats.lessonsCompleted} label="Дууссан хичээл" />
          <StatCard icon={BookOpen} color="blue" value={stats.wordsLearned} label="Сурсан үг" />
        </div>
      </div>
        ...
        <div className="p-6 bg-card rounded-3xl border-2 border-border">
          <div className="grid grid-cols-7 gap-2">
            {['Ня', 'Да', 'Мя', 'Лх', 'Пү', 'Ба', 'Бя'].map((day, i) => (
              <div key={i} className="text-center text-sm font-bold text-muted-foreground pb-2">
                {day}
              </div>
            ))}
